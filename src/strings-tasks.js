...
function removeFirstOccurrences(str, value) {
  const currentIndex = str.indexOf(value);
  if (currentIndex < 0) return str;
  const firstPart = str.slice(0, currentIndex);
  const secondPart = str.slice(currentIndex + value.length);
...
function removeLastOccurrences(str, value) {
  const currentIndex = str.lastIndexOf(value);
  if (currentIndex < 0) return str;
  const firstPart = str.slice(0, currentIndex);
  const secondPart = str.slice(currentIndex + value.length);
  return firstPart.concat(secondPart);
...
function countVowels(str) {
  ...
  const wrodBook = 'aeiouy';

  workStr.forEach((latter) => {
    if (wrodBook.includes(latter.toLowerCase())) {
      result += 1;
    }
  });
...
function isPalindrome(str) {
  const normalizeStr = str
    ...
    .join('')
    .toLowerCase();
  const reverseStr2 = normalizeStr.split('').reverse().join('');

  const isPalindrom = normalizeStr === reverseStr2;
  return isPalindrom;
}
...
function findLongestWord(sentence) {
  ...
  sentence.split(' ').forEach((elem) => {
    if (result.length < elem.length) result = elem;
  });
  return result;
}
...
function reverseWords(str) {
  ...
  str.split(' ').forEach((elem) => {
    result = result.concat(` ${elem.split('').reverse().join('')}`).trim();
  });
  return result;
}
